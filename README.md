...
* `GET /bookings?status=pending&skip=0&limit=10` - получить список броней;
...
**Retry/backoff** добавлен для имитации временного сбоя внешнего сервиса. Worker повторяет задачу с экспоненциальной задержкой, а после исчерпания попыток переводит бронь в `failed`.

**Rate limiting** на `POST /bookings` реализован простым in-memory ограничителем: не более 5 запросов в минуту с одного клиента.

**Structured logging** выводит логи приложения и worker-а в JSON-формате, чтобы их было удобно читать в Docker и собирать внешними системами.
